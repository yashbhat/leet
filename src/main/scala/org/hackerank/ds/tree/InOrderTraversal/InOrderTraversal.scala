package org.hackerank.ds.tree.InOrderTraversal

import scala.io.StdIn

object Solution {

  private def inOrderTraversal(treeNode: BinTreeNode, curr: Seq[String] = Seq()): Seq[String] =
    treeNode match {
      case BinTreeNode(None, _, _)                    => curr
      case BinTreeNode(Some(value), None, None)       => curr.appended(value.toString)
      case BinTreeNode(Some(value), Some(tree), None) => inOrderTraversal(tree, curr).appended(value.toString)
      case BinTreeNode(Some(value), None, Some(tree)) => inOrderTraversal(tree, curr.appended(value.toString))
      case BinTreeNode(Some(value), Some(treeL), Some(treeR)) =>
        inOrderTraversal(treeR, inOrderTraversal(treeL, curr).appended(value.toString))
    }
